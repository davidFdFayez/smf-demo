...
export async function listAllNews(signal?: AbortSignal): Promise<NewsArticleSummary[]> {
  const { data } = await httpClient.get<NewsArticleSummary[]>("/api/news/admin", { signal });
  return data;
}
...
export async function createNewsArticle(input: CreateNewsInput): Promise<NewsArticleDetails> {
  const { data } = await httpClient.post<NewsArticleDetails>("/api/news", input);
  return data;
}
...
export async function updateNewsArticle(
  ...
  const { data } = await httpClient.put<NewsArticleDetails>(
...
export async function getNewsBySlugAdmin(
  ...
  const { data } = await httpClient.get<NewsArticleDetails>(
    `/api/news/admin/${encodeURIComponent(slug)}`,
    { signal },
  );
  return data;
}
